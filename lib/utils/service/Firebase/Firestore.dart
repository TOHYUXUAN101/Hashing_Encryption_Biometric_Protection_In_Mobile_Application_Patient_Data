...
class FireStore {
  static Future<bool> register({
    ...
    try {
      ...
      DocumentReference docRef = await user.add({
        ...
        'UserInfoEncKey':'',
        'UserInfoEncIV':'',
        'UserRegisteredDeviceID': deviceUniqueID
      });
  ...
  static bool signIn({
    ...
    required String deviceUniqueID,
    required String deviceUniqueIDRegistered,
  }){

    if(userNewlyHashed == userPreviouslyHashed && userIsValidated){
      if(deviceUniqueID == deviceUniqueIDRegistered){
        return true;
      }else{
        // here will have some process to email verification for changing device purpose, I simply put the dialog to show that will have some process in that
        ScreenUtils.showCupertinoAlertDialog(
        context, 

        "Warning",
        "Are you Using Different Device to login? "
        );
        return false;
      }
    }
    ...
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        if (docUserEmail == userEmail) {
          ...
          String docUserRegisteredDeviceID = doc['UserRegisteredDeviceID'];
          
          UserDataRetrieve userDataRetrieve = UserDataRetrieve(uID: docUserID, uMail: docUserEmail, uKey: docUserHashKey, uPassHashed: docUserHash, uIsValidated: docUserValidated, uDeviceID: docUserRegisteredDeviceID);
